require "test_helper"

class Auth::AuthenticateWithExercismTest < ActiveSupport::TestCase
  test "creates new user from Exercism token" do
    exercism_payload = {
      'id' => '1530',
      'email' => 'newuser@exercism.org',
      'name' => 'New User',
      'handle' => 'iHiD',
      'avatar_url' => nil
    }

    Auth::VerifyExercismToken.stubs(:call).returns(exercism_payload)

    assert_difference 'User.count', 1 do
      user = Auth::AuthenticateWithExercism.('exercism-code', 'code-verifier')
      assert_equal 'newuser@exercism.org', user.email
      assert_equal 'New User', user.name
      assert_equal '1530', user.exercism_id
      assert user.uses_oauth?
      assert user.confirmed?
      assert_equal 'ihid', user.handle
    end
  end

  test "falls back to generated handle when Exercism handle is taken" do
    create(:user, handle: 'ihid')

    exercism_payload = {
      'id' => '1530',
      'email' => 'jeremy@exercism.org',
      'name' => 'Jeremy Walker',
      'handle' => 'iHiD',
      'avatar_url' => nil
    }

    Auth::VerifyExercismToken.stubs(:call).returns(exercism_payload)

    user = Auth::AuthenticateWithExercism.('exercism-code', 'code-verifier')

    # Falls back to generating a handle from the email
    assert_equal 'jeremy', user.handle
  end

  test "falls back to generated handle when Exercism handle is blank" do
    exercism_payload = {
      'id' => '1530',
      'email' => 'someone@exercism.org',
      'name' => 'Someone',
      'handle' => nil,
      'avatar_url' => nil
    }

    Auth::VerifyExercismToken.stubs(:call).returns(exercism_payload)

    user = Auth::AuthenticateWithExercism.('exercism-code', 'code-verifier')

    assert_equal 'someone', user.handle
  end

  test "finds existing user by exercism_id" do
    existing_user = create(:user,
      email: 'existing@exercism.org',
      exercism_id: '456')

    exercism_payload = {
      'id' => '456',
      'email' => 'existing@exercism.org',
      'name' => 'Existing User',
      'handle' => 'existing',
      'avatar_url' => nil
    }

    Auth::VerifyExercismToken.stubs(:call).returns(exercism_payload)

    assert_no_difference 'User.count' do
      user = Auth::AuthenticateWithExercism.('exercism-code', 'code-verifier')

      assert_equal existing_user.id, user.id
    end
  end

  test "links existing user by email when exercism_id not found" do
    existing_user = create(:user,
      email: 'existing@exercism.org',
      exercism_id: nil,
      confirmed_at: nil)

    exercism_payload = {
      'id' => '789',
      'email' => 'existing@exercism.org',
      'name' => 'Existing User',
      'handle' => 'existing',
      'avatar_url' => nil
    }

    Auth::VerifyExercismToken.stubs(:call).returns(exercism_payload)

    assert_no_difference 'User.count' do
      user = Auth::AuthenticateWithExercism.('exercism-code', 'code-verifier')
      assert_equal existing_user.id, user.id
      assert_equal '789', user.exercism_id
      assert user.uses_oauth?
      assert user.confirmed?
    end
  end

  test "defers avatar copy for new user with avatar_url" do
    exercism_payload = {
      'id' => '1530',
      'email' => 'avatar@exercism.org',
      'name' => 'Avatar User',
      'handle' => 'avataruser',
      'avatar_url' => 'https://exercism.org/avatars/1530/0'
    }

    Auth::VerifyExercismToken.stubs(:call).returns(exercism_payload)

    User::Avatar::CopyFromUrl.expects(:defer).with do |user, url|
      user.email == 'avatar@exercism.org' && url == 'https://exercism.org/avatars/1530/0'
    end

    Auth::AuthenticateWithExercism.('exercism-code', 'code-verifier')
  end

  test "does not defer avatar copy when avatar_url is blank" do
    exercism_payload = {
      'id' => '1530',
      'email' => 'noavatar@exercism.org',
      'name' => 'No Avatar',
      'handle' => 'noavatar',
      'avatar_url' => nil
    }

    Auth::VerifyExercismToken.stubs(:call).returns(exercism_payload)

    User::Avatar::CopyFromUrl.expects(:defer).never

    Auth::AuthenticateWithExercism.('exercism-code', 'code-verifier')
  end

  test "does not defer avatar copy when linking existing user by email" do
    create(:user, email: 'existing@exercism.org', exercism_id: nil)

    exercism_payload = {
      'id' => '789',
      'email' => 'existing@exercism.org',
      'name' => 'Existing User',
      'handle' => 'existing',
      'avatar_url' => 'https://exercism.org/avatars/789/0'
    }

    Auth::VerifyExercismToken.stubs(:call).returns(exercism_payload)

    User::Avatar::CopyFromUrl.expects(:defer).never

    Auth::AuthenticateWithExercism.('exercism-code', 'code-verifier')
  end

  test "generates random password for OAuth users" do
    exercism_payload = {
      'id' => '1530',
      'email' => 'password@exercism.org',
      'name' => 'Password Test',
      'handle' => 'passworduser',
      'avatar_url' => nil
    }

    Auth::VerifyExercismToken.stubs(:call).returns(exercism_payload)

    user = Auth::AuthenticateWithExercism.('exercism-code', 'code-verifier')

    # User should have an encrypted password (random generated)
    assert user.encrypted_password.present?
    # Should not be able to login with empty password
    refute user.valid_password?("")
  end
end
