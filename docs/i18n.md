...
- **One exception: `Level` milestone email copy is DB-backed**, seeded from
  `db/seeds/level_translations/*.json` (one file per locale), validated by
...
**Seeding:** `db/seeds/level_translations/` holds one file per locale, named
after the locale it holds (e.g. `hu.json`) - not one giant file for every
locale, to keep individual files reviewable. Each file is an array of
`{ level_uuid, milestone_email_subject, milestone_email_content_markdown }`
entries (no `locale` key - it's derived from the filename), matched to
`Level` by `uuid` (not slug - mirrors how `Level::CreateAllFromJson`
matches). `db/seeds.rb` globs every file in the directory and loads each
through `Level::Translation::CreateAllFromJson`, called after the
...
The `hu.json` entries are currently placeholders (prefixed `[HU TBD]`), not
real translations - this PR built the seeding/validation mechanism, not the
translation content. Replacing them with real copy (by hand or via
`Level::Translation::TranslateToLocale` / `TranslateToAllLocales`, the
existing Gemini-backed commands) is separate follow-up work. Adding a new
locale means adding a new `{locale}.json` file to the directory - no code
changes needed.

**Validation:** `test/db/seeds/level_translations_completeness_test.rb`
checks that every level in `curriculum.json` has a complete entry in every
`level_translations/*.json` file, with the same production-hard-fail /
other-locales-warn severity split as `i18n_parity_test.rb`. This runs as
part of `bin/rails test` too.
